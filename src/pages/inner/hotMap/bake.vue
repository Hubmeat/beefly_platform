      ...
      <el-form :inline="true">
       ...
       <el-row class="showTime">
        <el-col class="dateArrow" ref="dateArrow" v-bind:data-timetype="arrowTimeType">
          <el-button @click="dateMinus" v-show="arrowShow">
            <i class="el-icon-arrow-left"></i>
          </el-button>
          <span class="nowTime">{{nowTime}}</span>
          <el-button @click="dateAplus" v-show="arrowShow" >
        ...
        <el-col>
		  ...
		  <div id="map-container"></div>
            ...
            <!-- <div class="drivingPanel" v-show="true">
...
div.showTime{margin-bottom:20px;}
...
#hotmap_controller .hotmap_button {
    display: block;
    width: 100%;
    height: 36px;
    line-height: 36px;
...
#hotmap_controller .hotmap_button button { 
    width: 100px;
    height: 30px;
    line-height: 30px;
    display: inline-block;
...
#hotmap_controller .hotmap_button button:nth-of-type(1) { 
    width: 100px;
    height: 30px;
    line-height: 33px;
    margin-left: 8px;
...
#hotmap_controller .hotmap_button button:hover {
    color: #fff;
    background: rgb(66,66,66);
...
#hotmap_controller .hotmap_search {
    display: block;
    width: 100%;
    height: 40px;
    line-height: 40px;
...
#hotmap_controller .hotmap_search button {
    width: 80px;
    height: 36px;
    float: right;
    line-height: 11px;
...
export default {
  ...
  mounted: function () {
    ...
    var dateType = this.$refs.dateArrow.$el.dataset.timetype
    if (dateType === 'real-time') {
      this.timer = setInterval(function () {
        that.nowTime = moment().format('YYYY-MM-DD HH:MM:ss')
      }, 1000)
      this.arrowTimeType = 'real-time'
    ...
    dateMinus () {
      ...
      switch (dateTimeType) {
        case 'day': {
          ...
          lastDay = new Date().getTime() + 24 * 60 * 60 * 1000 * nums
    ...
    handleChangeType (e) {
      ...
      switch (e.currentTarget.innerText) {
        ...
        case '本周': {
          clearInterval(this.timer)
          this.arrowShow = true
          this.nowTime = moment().format('YYYY年第ww周')
        ...
        case '本月': {
          clearInterval(this.timer)
          this.arrowShow = true
          this.nowTime = moment().format('YYYY年MM月')
